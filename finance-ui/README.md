...
| 模块 | 说明 | 数据来源 |
|------|------|---------|
| 智能分析 | 根据储蓄率、最大支出类别、收支比生成 3-4 条洞察文案 | 前端本地计算（基于真实账单数据） |
| KPI 卡片 | 本月收入、本月支出、净现金流、储蓄率 | 全部来自后端 API |
| 收支对比面积图 | 本月每日收入与支出趋势 | 基于真实账单数据按日期聚合 |
...
| 即将到期账单 | 固定支出提醒（待开发，预留 UI 空间） | 暂无后端接口 |
| 预算进度 | 分类预算使用进度条（待开发，预留 UI 空间） | 暂无后端接口 |
...
- **智能分析**（`FinanceDashboard.vue`）：根据储蓄率、最大支出类别、收入支出比自动生成个性化财务建议，支持 4 种语气等级
...
- **中英双语**：登录页和仪表盘均支持中/英文切换，智能分析、预算、KPI 等文案均有对应翻译
- **侧边栏导航**（`Sidebar.vue`）：7 个菜单项（仪表盘 / 交易记录 / 预算 / 分析 / 投资 / 智能分析 / 设置），带图标和徽章
