<template>
  ...
  <div v-else class="app-layout">
    <Sidebar :current-user="currentUser" :bills-count="billsCount" @logout="logout" />
    <main class="main-content">
      <FinanceDashboard :current-user="currentUser" @logout="logout" @bills-change="onBillsChange" />
    </main>
  </div>
</template>
...
const handleAuthenticated = (payload) => { currentUser.value = payload.user; setLanguage(payload.language) }
const logout = () => { localStorage.removeItem('pf_user'); currentUser.value = null; billsCount.value = 0 }
const onBillsChange = (count) => { billsCount.value = count }
</script>
